...
namespace Demo.Test
{
    ...
    {
        private readonly Mock<IConnectorService> connectorServiceStub = new();
        ...
        public async Task AddConnectorToChargeStation_WhenGroupHasEnoughCapacity_ShouldReturnConnector()
        {
            ...
            connectorServiceStub.Setup(p => p.AddConnectorToChargeStation(1, connector))
                .ReturnsAsync(new APIResponse<Connector> { Data = connector });
        ...
        public async Task AddConnectorToChargeStation_whenGroupHasNotEnoughCapacity_ShouldReturnBadRequest()
        {
            ...
            connectorServiceStub.Setup(p => p.AddConnectorToChargeStation(1, connector))
                .ReturnsAsync(new APIResponse<Connector> { Data = null });
            var controller = new ConnectorController(connectorServiceStub.Object);
            var addedConnector = await controller.PostConnector(1, connector);
        ...
        {
            connectorServiceStub.Setup(p => p.DeleteConnector(2, 1))
                .ReturnsAsync(new Connector { ChargeStationId = 1, ConnectorId = 2 });
            var controller = new ConnectorController(connectorServiceStub.Object);
        ...
        public async Task UpdateConnector()
        {
            ...
            connectorServiceStub.Setup(p => p.UpdateCurrent(2, 1, 40)).ReturnsAsync(new APIResponse<Connector>
                { Data = connector, Succeeded = true });
            var controller = new ConnectorController(connectorServiceStub.Object);
